import { ChangeDetectionStrategy, Component, EventEmitter, Output, inject, input, signal, computed, effect } from '@angular/core';
  ...
  template: `
    <app-widget-frame [title]="config().title" [loading]="resource.isLoading()" (remove)="remove.emit()">
      <div frame-controls class="table-controls">
        ...
        <button type="button" class="table-controls__export" title="Export CSV" (click)="exportCsv()">CSV</button>
        <button type="button" class="table-controls__export" title="Export PDF" (click)="exportPdf()">PDF</button>
      </div>
      ...
      <div class="table-widget__toolbar">
        ...
        />
        <span class="table-widget__count">{{ filteredRows().length }} results</span>
      </div>
      ...
      @if (resource.error()) {
          ...
          <table class="table-widget">
            <thead>
              <tr>
                ...
                <th (click)="sortBy('country')">Country{{ sortIndicator('country') }}</th>
                <th (click)="sortBy('date')">Date{{ sortIndicator('date') }}</th>
                <th class="table-widget__num" (click)="sortBy('metric')">{{ metricLabel() }}{{ sortIndicator('metric') }}</th>
              </tr>
            </thead>
            <tbody>
              @for (row of pagedRows(); track row.id) {
                <tr>
                  ...
                  <td class="table-widget__num">{{ formatMetric(row.metric) }}</td>
                </tr>
              } @empty {
                <tr>
                  <td colspan="5" class="table-widget__empty">No rows match your filter.</td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <div class="table-widget__pagination">
          ...
          <div class="table-widget__pager">
            <button type="button" [disabled]="page() === 0" (click)="page.set(page() - 1)">‹</button>
            <button type="button" [disabled]="page() >= totalPages() - 1" (click)="page.set(page() + 1)">›</button>
          </div>
        </div>
      }
    </app-widget-frame>
...
export class TableWidgetComponent {
  ...
  readonly resource = rxResource({
    ...
    loader: ({ request }) => this.mockData.getTableRows(request.dataset, request.range),
  });
  ...
  readonly filteredRows = computed(() => {
    ...
    const filtered = q
      ? rows.filter((r) => r.name.toLowerCase().includes(q) || r.email.toLowerCase().includes(q) || r.country.toLowerCase().includes(q))
      : rows;
  ...
  formatMetric(value: number): string {
    return formatByUnit(value, DATASET_META[this.config().settings.dataset].unit);
  }
  ...
  exportPdf(): void {
    ...
    this.exportSvc.exportRowsToPdf(
      ...
      this.filteredRows().map((r) => ({ ...r, metric: this.formatMetric(r.metric) })),
    );
  }
}
